...
for /r "%INPUT_DIR%" %%F in (*.png, *.jpg, *.jpeg, *.bmp, *.tga, *.gif) do (
	...
	:: Применяем удаления прозрачности к увеличенному RGBA
	"%MAGICK_PATH%magick.exe" "!TEMP_RGBA!" -alpha opaque -transparent black "!TEMP_RGBA!" || (
		echo Ошибка при удаления прозрачности: !RELATIVE_PATH!
		pause
		exit /b 1
	)
